...
namespace EconomyGraph.Views.ContentViews
{
    ...
    public partial class ShadedMultiLineGraphView : MultiLineGraphView
    {
        public ShadedMultiLineGraphView()
        {
            InitializeComponent();
        }
        ...
        {
            ShadedMultiLineGraphViewModel viewModel = ViewModel as ShadedMultiLineGraphViewModel;
            ...
            foreach (MultiLineDataGroup dg in viewModel.DataGroups)
            {
                foreach(var dataPoint in dg.Lines[0].DataPoints)
                {
                    ...
                    xDataPointEnd += dg.GroupWidth / dg.Lines[0].DataPoints.Count;
                    if (first)
                    {
                        first = false;
                    ...
                    foreach(var shadePeriod in viewModel.ShadePeriods)
                    {
                        ...
                        else
                        {
                            ...
                            shadeXStart = (startDate / dpDays) * (dg.GroupWidth / dg.Lines[0].DataPoints.Count) + xDataPointStart - 1;
                        }
